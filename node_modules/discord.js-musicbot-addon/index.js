...
try {
    ...
    */
    class Music {
      ...
      */
      ...
      isAdmin(member) {
        ...
        return member.hasPermission("ADMINISTRATEUR");
      };
      ...
      setLast(server, last) {
        return new Promise((resolve, reject) => {
          if (this.queues.has(server)) {
            ...
            reject("pas de file d'attente du serveur");
          };
        });
      };
      ...
      emptyQueue(server) {
        return new Promise((resolve, reject) => {
          if (!musicbot.queues.has(server)) reject(new Error(`[Queue vide] Aucune file d'attente trouvée pour ${server}`));
      ...
      updatePresence(queue, client, clear) {
        return new Promise((resolve, reject) => {
          if (!queue || !client) reject("arguments non valides");
    ...
    client.on("ready", () => {
      ...
      if (musicbot.cooldown.exclude.includes("skip")) console.warn(`[MUSIQUE] L'exclusion de SKIP CMD des délais de récupération peut entraîner des problèmes.`);
      if (musicbot.cooldown.exclude.includes("play")) console.warn(`[MUSIQUE] Exclure PLAY CMD des délais de récupération peut entraîner des problèmes.`);
      if (musicbot.cooldown.exclude.includes("remove")) console.warn(`[MUSIQUE] L'exclusion de REMOVE CMD du temps de recharge peut entraîner des problèmes.`);
      if (musicbot.cooldown.exclude.includes("search")) console.warn(`[MUSIQUE] L'exclusion de SEARCH CMD du temps de recharge peut entraîner des problèmes.`);
      setTimeout(() => { if (musicbot.musicPresence == true && musicbot.client.guilds.length > 1) console.warn(`[MUSIQUE] MusicPresence est activé avec plusieurs serveurs!`); }, 2000);
    });
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        ...
        ytpl(playid, function(err, playlist) {
          ...
          playlist.items.forEach(video => {
            ...
            if (ran >= playlist.items.length) {
              ...
              else if (index == 1) msg.channel.send(musicbot.note('note', `Mis en file d'attente une chanson.`));
              else if (index > 1) msg.channel.send(musicbot.note('note', `En file d'attente ${index}.`));
            }
          });
        });
      } else {
        ...
        new Promise(async (resolve, reject) => {
          ...
          if (!res) return msg.channel.send(musicbot.note("fail", "Quelque chose s'est mal passé. Réessayer!"));
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            ...
            try {
              ...
              if (musicbot.requesterName && !resMem) embed.setFooter(`Demandé par \`UnknownUser (ID: ${res.requester})\``, res.requesterAvatarURL);
              msg.channel.send({
                embed
              });
            } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me)
          .has('EMBED_LINKS')) {
          const embed = new Discord.RichEmbed();
          embed.addField(`:notes: Commandes de musique :notes:`, `Utilisation \`${musicbot.botPrefix}${musicbot.help.name} nom de la commande\` pour obtenir de l'aide sur l'utilisation. Toute personne ayant un rôle nommé \`${musicbot.djRole}\` peut utiliser n'importe quelle commande.`);
          ...
          var cmdmsg = `= Commandes de musique =\nUtilisation ${musicbot.botPrefix}${musicbot.help.name} [commande] pour obtenir de l'aide sur une commande. Toute personne ayant un rôle nommé \`${musicbot.djRole}\` peut utiliser n'importe quelle commande.\n`;
          let index = 0;
          let max = musicbot.commandsArray.length;
          for (var i = 0; i < musicbot.commandsArray.length; i++) {
    ...
    musicbot.skipFunction = (msg, suffix, args) => {
      ...
      const queue = musicbot.getQueue(msg.guild.id);
      if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `Vous ne pouvez pas sauter ceci car vous ne l'avez pas fait la queue.`));

      if (musicbot.queues.get(msg.guild.id).loop == "song") return msg.channel.send(musicbot.note("fail", "Ne peut pas ignorer quand la boucle est définie sur single."));
    ...
    musicbot.pauseFunction = (msg, suffix, args) => {
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (dispatcher.paused) return msg.channel.send(musicbot.note(`fail`, `Musique déjà en pause!`))
      else dispatcher.pause();
      msg.channel.send(musicbot.note('note', 'Lecture en pause.'));
    };
    ...
    musicbot.resumeFunction = (msg, suffix, args) => {
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher.paused) return msg.channel.send(musicbot.note('fail', `La musique joue déjà.`))
      else dispatcher.resume();
      msg.channel.send(musicbot.note('note', 'La lecture reprise.'));
    };
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (msg.channel.permissionsFor(msg.guild.me)
        ...
        try {
          ...
          if (musicbot.requesterName && !resMem) embed.setFooter(`Demandé par \`UnknownUser (ID: ${queue.last.requester})\``, queue.last.requesterAvatarURL);
          msg.channel.send({
            embed
          });
        } catch (e) {
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      ...
      if (suffix) {
        ...
        if (musicbot.requesterName && !resMem) embed.setFooter(`Demandé par \`UnknownUser (ID: ${video.requester})\``, video.requesterAvatarURL);
        ...
        if (queue.songs.length > 11) {
          ...
          embed.setColor(musicbot.embedColor);
          embed.setFooter(`Page ${page} de ${pages.length}`);
          ...
          msg.channel.send(embed).then(m => {
            m.react('⏪').then( r => {
              ...
              forwardsFilter.on('collect', r => {
                ...
                page++;
                embed.setDescription(pages[page - 1]);
                embed.setFooter(`Page ${page} de ${pages.length}`, msg.author.displayAvatarURL);
                m.edit(embed);
              })
                  ...
                  .then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (song_number >= 0) {
                        ...
                        if (musicbot.requesterName && !resMem) embed.setFooter(`Demandé par \`UnknownUser (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
    ...
    musicbot.volumeFunction = (msg, suffix, args) => {
      ...
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'Aucun volume spécifié.'));
    ...
    musicbot.clearFunction = (msg, suffix, args) => {
      ...
      if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", `Seuls les admins ou les personnes ayant le ${musicbot.djRole} peut effacer les files d'attente.`));
      musicbot.emptyQueue(msg.guild.id).then(res => {
        ...
        if (voiceConnection !== null) {
          ...
          if (!dispatcher || dispatcher === null) {
            if (musicbot.logging) return console.log(new Error(`dispatcher null on skip cmd [${msg.guild.name}] [${msg.author.username}]`));
            return msg.channel.send(musicbot.note("fail", "Quelque chose a mal tourné."));
          };
          if (voiceConnection.paused) dispatcher.end();
          dispatcher.end();
        }
        ...
        return msg.channel.send(musicbot.note("fail", "Quelque chose a mal tourné la file d'attente."));
      })
    };
    ...
    musicbot.removeFunction = (msg, suffix, args) => {
      ...
      if (test) {
        if (test.requester !== msg.author.id && !musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", "Vous ne pouvez pas enlever cet article."));
      ...
      new Promise((resolve, reject) => {
          ...
          if (musicbot.messageNewSong == true && queue.last && musicbot.queues.get(msg.guild.id).loop !== "song") {
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              ...
              .setColor(musicbot.embedColor)
              .setFooter(`Demandé par ${req !== null ? req.username : "Unknwon User"}`, `${req !== null ? req.displayAvatarURL : null}`);
              msg.channel.send({embed});
            } else {
          ...
          try {
            ...
            connection.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Quelque chose s'est mal passé avec la connexion. Réessayer la file d'attente ...`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });

            dispatcher.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Quelque chose a mal tourné en jouant de la musique. Réessayer la file d'attente ...`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });
                ...
                if (musicbot.queues.get(msg.guild.id).songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    ...
                    musicbot.updatePositions(musicbot.queues.get(msg.guild.id).songs, msg.guild.id).then(res => {
                      musicbot.queues.get(msg.guild.id).songs = res;
                      musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                    }).catch(() => { console.error(new Error("quelque chose a mal tourné la queue")); });
                  } else if (loop == "queue" || loop == "song") {
                    musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                  };
                } else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) {
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', 'Lecture terminée.'));
                  musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                  if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                  const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
                  if (voiceConnection !== null) return voiceConnection.disconnect();
